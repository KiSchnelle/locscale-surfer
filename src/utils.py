...
def convert_to_tuple(input_variable, num_dims=3):
    ...
    if hasattr(input_variable, "__iter__"):
        if len(input_variable) == num_dims:
            ...
            print(
                "Input variable dimension {} doesn't match expected output dimension {}".format(
                    len(input_variable), num_dims
                )
            )
    else:
...
def save_as_mrc(
    map_data, output_filename, apix=None, origin=None, verbose=False, header=None
):
    """
    ...
    with mrcfile.new(output_filename, overwrite=True) as mrc:
        ...
        if header is not None:
            ...
            if apix is not None:
                ...
                rec_array_apix = np.rec.array(
                    apix_tuple, dtype=[("x", "<f4"), ("y", "<f4"), ("z", "<f4")]
                )
            ...
            rec_array_origin = np.rec.array(
                origin_tuple, dtype=[("x", "<f4"), ("y", "<f4"), ("z", "<f4")]
            )
...
def resample_map(
    emmap, emmap_size_new=None, apix=None, apix_new=None, order=1, assert_shape=None
):
    """
    ...
    if emmap_size_new is None:
        if apix is not None and apix_new is not None:
            ...
            raise UserWarning(
                "Provide either (1) current pixel size and new pixel size or (2) new emmap size"
            )
        ...
        try:
            ...
            raise UserWarning(
                "Please provide proper input: emmap_size_new must be a tuple"
            )
    ...
    for i in range(0, length, step_size):
        for j in range(0, width, step_size):
            for k in range(0, height, step_size):
                ...
                if (
                    k + cube_size < length
                    and j + cube_size < width
                    and i + cube_size < height
                ):
...
def filter_cubecenters_by_mask(
    cubecenters, mask, cube_size, signal_to_noise_cubes=None
):
    """
    ...
    if signal_to_noise_cubes is not None:
        ...
        if num_noise_cubes < required_noise_cubes:
            ...
            print(
                f"Using {required_noise_cubes} noise cubes out of {num_noise_cubes} noise cubes randomly"
            )
            sampled_noise_cubes = random.sample(
                noise_cubes_centers, required_noise_cubes
            )
